...
- 界面展示服务端真实 `stage`、已处理时长和暂时发现的动作数量，例如“已处理 00:42 / 01:00，暂时发现 3 个动作”。
...
- `discovered_candidate_count` 只驱动“暂时发现 N 个动作”。中间候选只读，不能预览、编辑或加入方案，直到本次运行进入可靠终态。
...
本地入口的公开错误包括：功能关闭 `404`、不支持的媒体类型 `415`、文件过大 `413`、ID／媒体时长／区间无效 `422`，以及现有的会话 `403`、容量 `429` 和就绪 `503`。错误响应不暴露本机路径、Provider 正文或内部请求数据。
...
- `source_duration_seconds` 是有限正数且始终表示原视频完整时长；`processed_seconds` 是有限非负数，区间运行时最大为区间长度；`discovered_candidate_count` 是非负整数。
...
4. 过程预览随服务端快照推进，暂时候选不可编辑或加入方案，终态后才开放校正。
5. 构造中间一分钟失败的部分结果：两侧可靠候选可使用，缺口范围清晰；只重试该区间成功后按绝对时间合并并移除缺口。
