...
CloudBase 平台启动探针必须设置 `InitialDelaySeconds=60`。五来源生产等价同步实测约 19 秒；同步器的 240 秒全局截止只负责失败关闭，不能解释为平台会等待同样久。真实 `CreateVersion` 已证明 300 秒探针延迟会先触发平台约 240 秒管理超时，因此候选必须在 60 秒探针窗口内完成媒体同步并启动 Uvicorn。发布回执记录该非秘密配置值，且不能把单次媒体读取超时或重试次数相加后冒充平台启动上限。
