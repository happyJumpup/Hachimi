...
> 状态：CloudBase Run 版本 A/B、私有真实视频、公网并发 canary 与 B→A→B 回滚均已通过；GYMTI/七猫最终候选已完成本地集成，但 `trainpal-demo-011` 的一次性定向路由没有在数据面命中候选，因此未替换 `009`；最终素材、移动真机与录屏仍待团队验收
>
> 更新日期：2026-07-23
>
...
- 最终候选本地门禁已完成 Web 202、API 290、Playwright 20/20，以及 PowerShell／Git Bash 对同一镜像的资源、FFmpeg、生产 `/ready` 和匿名 GYMTI 零模型调用审计。CloudBase `trainpal-demo-011` 构建与提交身份通过，但请求头和 URL 参数的控制面规则都没有使数据面 `/health` 命中候选；门禁在上传与模型调用前失败，恢复 `009=100%`，没有执行公开切换或候选回滚。
...
- 腾讯云确认当前服务可用的零百分比候选定向访问方式；在数据面身份门禁可复现通过前，不重试 `011` 公网发布。

因此当前状态是“短期公网评委入口已通过工程 canary”，不是长期生产发布，也不代表任意健身视频都能准确解析。候选失败但 `009` 健康时只恢复 FLOW 100%／候选 0%；仅稳定公网受影响、发生秘密或原始内容泄露，或触发预算／到期关停时关闭公网。
...
- 已有公开稳定版 `009` 后，候选私有门禁必须保留 `OA + PUBLIC` 和现有映射；候选保持 0%，仅一次性请求头命中候选，普通访问继续落到 `009`。
...
- 候选 canary 失败立即恢复 `009` 的 FLOW 100%／候选 0%，不得关闭当前公网入口；只有首次 OA-only 基础版本失败时才关闭公网开关与映射。把最小实例恢复为 0 只能作为后续成本动作。
...
2. 使用 `release-source.ps1` 从干净且不可变的完整提交生成源码包，再通过已验证的 CloudBase CLI 3.6.4 提交 `trainpal-demo` 的 0% `GRAY` 候选；若稳定版已经公开，脚本必须保留 `OA + PUBLIC`，不得为制造“私有”状态中断当前链接。
...
2. 使用 `run-private-canary.ps1` 执行一次性随机请求头定向灰度，稳定版保持默认路由；同一请求头必须贯穿健康身份、JSON、上传与 SSE。脚本必须确认 `/health` 的精确候选提交、轮询确认云端请求头路由已生效，并在 `finally` 中恢复且验证稳定版 100%／候选 0%。恢复失败按 P0 阻断发布。
3. CI 与私有门禁全部通过后，短时切换候选并验证第二浏览器、三路并发、超额 429；随后完成候选→稳定版→候选的真实流量回滚，再结束灰度。
4. 任何失败都恢复稳定版并放弃灰度；全部通过才保留新候选作为公开版本。请求头令牌只存在于发布进程内存，回执仅记录 SHA-256。
...
最终源码候选 `1da5882a094de3dd7f543e5c65d42a5f5d00b89d` 构建为 `trainpal-demo-011`，Build ID `2601400511`，保持 0% 百分比流量和 `0/1` 实例；第一次同 SHA 构建 `trainpal-demo-010`、Build ID `2601383583` 因构建层返回无效校验和格式失败。`011` 的控制面能够观测到唯一的一次性 Header 或 URL 参数规则，但签名私有网关与默认域名在最多 30 秒的数据面轮询中都继续返回稳定版身份。`run-private-canary.ps1` 在真实媒体上传前拒绝候选身份并恢复 `FLOW`，服务明细再次确认只有 `009=100%`。因此本次没有公开 `011`，也没有把未执行的 `011 → 009 → 011` 记作回滚成功。
...
候选门禁失败但稳定版 `009` 仍健康时，只恢复 `009` 的 FLOW 100%／候选 0%，不关闭当前入口。若故障已经影响稳定公网，或出现秘密／原始内容泄露，则在以下任一情况发生时立即关闭公网并回滚：`/ready` 持续失败、重复 5xx、SSE 串流／积压、训练主流程阻断、媒体哈希不符、临时文件持续增长、真实结果跨运行污染、实例因并发反复重启。

### 回滚步骤

1. 记录故障时间、当前 digest、配置修订、脱敏错误码和影响范围；不要先删除证据。
2. 若稳定公网受影响或发生泄露，关闭公网开关与映射并停止新分析；若仅候选失败，保持入口并确认普通请求仍落到 `009`。
