...
- 当前 production Provider 保持火山 ASR、Ark Seed 和确定性融合，但视觉改为连续 MP4 的 60 秒分块、10 秒重叠、单并发顺序处理，不以稀疏截图冒充完整窗口。内部视觉块以有界码率转码后直接作为 Ark Responses API 的 Base64 视频输入，不创建方舟 Files API 托管文件；超过 45,000,000 bytes 的块明确成为 `media_error` 覆盖缺口。语音与视觉共享 170 秒证据截止时间，并为 180 秒外层上限预留 10 秒终态和清理余量。旧 CloudBase 候选的短视频、295 秒样本、三路评委并发、清理与回滚记录只属于历史证据；当前公开单并发／600 秒频控候选尚未完成线上验收。生产环境不使用测试 Provider 或预置候选回退。
...
竞赛生产允许公开会话直接创建真实分析，不要求评委码。全局只保留一个公开分析槽，并对成功取得租约的请求同时按会话 ID 与服务端认可的客户端 IP 执行 600 秒一次的冷却；任一维度受限或槽被占用都立即返回 `429 + Retry-After`。详细合同见 [ADR-0043](../adr/0043-public-analysis-uses-one-slot-and-session-ip-cooldown.md)。
...
- 竞赛 GYMTI 模型增强使用 Ark／豆包最小数据调用：选题只发送稳定 ID 与服务端派生信号，结果叙事只发送既定结果 ID 和正向原因码。它使用独立的非排队三并发门，不依赖评委会话，也不占用公开真实分析槽或 600 秒尝试额度；第四条并发、模型失败或供应商留存边界未确认时立即使用确定性选题和模板叙事。
...
- `trainpal-demo-007` 至 `011`、旧公网 `009`、三路评委并发和 `008 → 007 → 008` 回滚均为 2026-07-23 早期候选的历史脱敏证据，不是当前最终版本声明。当前运行合同改为五条受控来源、公开单并发与 600 秒会话／IP 频控、Ark／豆包 GYMTI 独立三并发；精确当前候选仍须重新构建并通过 `/health` 身份、`/ready`、真实分析、降级和回滚门槛，未验证前不得宣称线上成功。
