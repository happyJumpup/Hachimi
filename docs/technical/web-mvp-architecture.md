...
- 当前 production Provider 保持火山 ASR、Ark Seed 和确定性融合，但视觉改为连续 MP4 的 60 秒分块、10 秒重叠、单并发顺序处理，不以稀疏截图冒充完整窗口。内部视觉块以有界码率转码后直接作为 Ark Responses API 的 Base64 视频输入，不创建方舟 Files API 托管文件；超过 45,000,000 bytes 的块明确成为 `media_error` 覆盖缺口。语音与视觉共享 170 秒证据截止时间，并为 180 秒外层上限预留 10 秒终态和清理余量。旧 CloudBase 候选的短视频、295 秒样本、三路评委并发、清理与回滚记录只属于历史证据；当前无冷却公开单并发候选尚未完成线上验收。生产环境不使用测试 Provider 或预置候选回退。
...
竞赛生产允许公开会话直接创建真实分析，不要求评委码。全局只保留一个公开分析槽，不按会话或 IP 记录完成后的次数额度；槽被占用时立即返回 `429 + Retry-After + X-TrainPal-Admission-Reason: capacity`，终态清理后同一会话可以立即再次创建。详细合同见 [ADR-0045](../adr/0045-public-analysis-keeps-one-slot-without-session-or-ip-cooldown.md)。
...
- 竞赛 GYMTI 模型增强使用 Ark／豆包最小数据调用：选题只发送稳定 ID 与服务端派生信号，结果叙事只发送既定结果 ID 和正向原因码。它使用独立的非排队三并发门，不依赖评委会话，也不占用公开真实分析槽；第四条并发、模型失败或供应商留存边界未确认时立即使用确定性选题和模板叙事。
...
- `trainpal-demo-007` 至 `011`、旧公网 `009`、三路评委并发和 `008 → 007 → 008` 回滚均为 2026-07-23 早期候选的历史脱敏证据，不是当前最终版本声明。当前运行合同改为五条受控来源、无冷却公开单并发、Ark／豆包 GYMTI 独立三并发；精确当前候选仍须重新构建并通过 `/health` 身份、`/ready`、真实分析、同会话重入、降级和回滚门槛，未验证前不得宣称线上成功。
- 新的公开内容理解结果、三个 TrainPal Skill、四态字段来源和个性化提案时效是冻结后的生产合同；其中 GYMTI 本地生命周期已落地，其余 Skill 持久化不因文档存在而视为完成。
- 签名 FFmpeg 镜像、收据和唯一二进制审计已有本地及历史 CloudBase 通过记录；历史公网三条真实并发结果均为 `partial`。当前五来源与新访问／GYMTI 合同的 `/ready` 尚须重新验证，界面必须继续显示覆盖缺口和“结果需要核对”。
