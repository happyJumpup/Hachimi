...
- Provider Profile 与受控来源代码上限为 5 分钟、256 MiB；CloudBase 公网请求 envelope 为 20 MB，因此用户本地上传和 `/capabilities` 固定在 19 MiB。用户界面只按 `GET /api/v1/capabilities` 返回的已发布时长边界开放，默认仍为 60 秒。只有真实五分钟 Canary 收据与当前部署 commit 匹配后，该接口才返回 300 秒。7／19 分钟原片仍须先在应用外裁成已发布上限内的新源文件。
- 过程反馈来自真实处理位置和暂时发现的动作线索数。部分结果必须明确覆盖缺口并允许逐段重试，不能把系统错误伪装成没有动作。
- 内容理解 Provider 使用完整豆包 ASR、连续静音 MP4 的 60 秒视觉块、10 秒重叠和确定性 EvidenceReconciler。评测选定的 Seed 主路线通过 Ark 直接内联视频；只有可降级错误才让当前及剩余失败块顺序切到通过门槛的 Qwen 备用，Qwen 通过 10 分钟只读签名 URL 读取私有 COS 临时对象。若质量证据只支持 Qwen 作为主路线，本轮安全边界会阻断版本 C。成功块不重跑，有效空结果不降级，COS 清理失败会关闭后续备用分析。
...
当前内容理解 Provider 继续使用豆包流式语音识别 2.0；Seed Mini 与 Seed Lite 的主路线选择及 Qwen3-VL 的备用资格必须由生产同构评测确定。竞赛基线已实现一次 ASR、60／10 秒顺序视觉分块、失败块降级和 `partial` 缺口；在真实 A/B/C 与 CloudBase Canary 前，默认仍只启用未冻结的 Ark 候选并发布 60 秒。
...
`local_analysis_max_seconds` 必须在 `(0, 300]` 内，并且只能等于当前 Canary 已验证的发布值；基础部署返回 `60`，绑定当前 commit 的五分钟收据通过后才返回 `300`。代码和私有验收上限不通过该公共字段暴露。`local_upload_max_bytes` 必须为正整数；功能关闭时仍返回发布边界，由 `local_upload_enabled` 单独控制创建能力。
