...
| 部署 Profile | 单实例、内存运行；Seed 主路线由评测选择，Qwen 备用默认关闭 |
| 对外能力 | 默认 60 秒；五分钟 Canary 收据与当前 commit 匹配后才可发布 300 秒 |
| 尚未交付 | 真实 A/B/C 冠军、真实跨厂商备用、CloudBase 三并发与回滚收据、10–30 分钟异步任务 |

当前媒体仍固定为 `1280px / 15fps / 4Mbps` 连续静音 MP4；紧凑 Profile 尚未完成同源 2pp／15% 门禁，因此没有切入生产。ASR 首版继续使用流式输入 2.0 与版本化健身热词；录音文件极速版只是后续独立候选，不在运行时自动降级，也没有被表述为已验证替代方案。
