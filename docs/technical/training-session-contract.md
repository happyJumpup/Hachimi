...
GYMTI 应用日志只能记录非内容型运行元数据：请求 ID、问卷／评分／叙事合同版本、模型名、耗时、成功／重试／降级状态、所选下一题 ID 和错误类别。不得记录请求正文、用户选项、得分或排名、原因码、提示词、模型原始响应、结果叙事、训练档案、身份信息或自由用户画像；异常对象和调试日志也必须经过同一字段白名单。模型供应商侧可配置的训练与内容留存必须关闭，不能满足这一生产边界的调用配置不得启用。竞赛生产环境默认关闭 GYMTI 模型；单独注入密钥不得启用调用，必须同时显式开启、确认供应商留存边界，并让现有评委会话通过并发与尝试预算门禁。匿名会话即使存在密钥也只能走确定性本地选题与模板叙事，不产生模型费用。
