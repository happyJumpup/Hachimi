...
2026-07-22 使用 `long-video-ab-v2` 完成了一次新的真实分级预检，本机总墙钟时间约
325 秒。ASR、Seed 联系表视觉和 Seed 融合通过了两条来源的全部预定门禁；Qwen3-VL
通过配置、最小调用和 2 秒合成探针，但在 7 分钟来源的 8 秒真实片段临时 OSS
multipart 上传阶段再次以 `qwen_upload_network_error_curl_exit_28` 失败，后续门禁
按协议跳过。回执共记录 46 项检查，其中 Qwen 失败 1 项、跳过 4 项。

该失败发生在 Qwen 模型推理和 Schema 校验之前，不能归因为识别质量。生命周期
journal 记录了 3 条脱敏边界，其中这次传输不明确对象产生 1 条 `failed` 记录；即时
本地审计因此为 `lifecycle_failed`，生产升级保持阻止。预检回执不含路径、密钥、
转录、对象键或原始响应；并发校准和 12 次完整 arm-run 均未启动，也没有宣布路线
胜出。下一步是在目标服务器或另一条已批准的显式 SOCKS 出口复跑相同 8 秒和 60 秒
门禁；不得用压缩输入或 Mock 偷换本轮 B 路线。
