...
脱敏报告只包含路线、冻结模型、Prompt 哈希、EvidenceReconciler 版本、聚合质量、成功率、延迟和明确的证据状态，不包含本地路径、对象 key、签名 URL、动作明细、Prompt 或模型原文。生产 readiness 必须拒绝由旧整理实现生成的报告。真实评测未完成时，生产默认继续使用既有 Ark 主路线并关闭跨厂商降级。
