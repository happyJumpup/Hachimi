...
4. In CloudBase Run, create `trainpal-demo` only after the console shows the full resource and price implications. Stop for fresh approval before submitting the resource-creation form. The published 360 CCU / CNY 76 reference rounds up to CNY 0.22 per CCU-hour; 2 CCU for the plan's maximum 18 warm hours is CNY 7.92. Requests, traffic, storage, builds, and add-ons must be checked separately in the console.
...
7. Confirm the actual demo end time. Enable a short, unannounced public canary and run public health/ready, judge SSE, three concurrent judge analyses, over-capacity 429, second-browser browse, and rollback-to-baseline smoke tests. Switch forward to the candidate again only after the rollback result is recorded. Disable public access immediately if a gate fails.
8. Announce the link only after the canary passes. Before judging, set the minimum instances to one.
9. The 2026-07-23 11:00 submission cutoff is not automatically the judging end. Disable the public-network switch and HTTP Access mapping at the confirmed demo end or 18 hours after first public enable, whichever is earlier; then return the minimum to zero and review unused billable resources.
